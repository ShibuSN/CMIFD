...
Total_no=0;
files = dir('*.jpg');
for numOfImg = 1:numel(files)
Total_no=Total_no+1;
filename = files(numOfImg).name;
show=1;
color=[64,0,128];
TR_P=2;
TR_sim=15;
...
num_match1=size(new_MatchList,1);
if num_match1==0
    disp('low matched points....');
    disp(filename);
    LowMatch_Total=LowMatch_Total+1;
    width =100;
    height =100;
    low_match=false;
    for i=1:num_match
...
%       Locations=cat(1,Locations1,Locations2);
    ...
    %   Matching (finding similar blocks)
     ...
     if(low_match)
        disp('Image Is Forged, With New algo');
        disp(filename);
        LowMatch_Detected=LowMatch_Detected+1;
     end
    return;
else
    disp('Image is forged...!!!');
    disp(filename);
    Forged_No=Forged_No+1;
end   
...
Match_superpixel=zeros(num_match1,2);%SR
...
fprintf('\nTotal image : %d\nForged : %d\nLowmatch total : %d\nLowmatch Correctly detected : %d\nNot Forged : %d\n',Total_no,Forged_No,LowMatch_Total,LowMatch_Detected,NotForged_No)
